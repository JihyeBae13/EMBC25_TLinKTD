...
codePath = pwd;
featurePath = [featurePath '\' dataName '\Data_from' num2str(tstart) '_to' num2str(tend)]; 
folderName = [classifierName 'source' featureName ];
...
saveas(h,fullfile(resultPath,['Fig_SucessRatesAll_TLsource' testRunName '.tif']))
saveas(h,fullfile(resultPath,['Fig_SucessRatesAll_TLsource' testRunName '.fig']))

% saving parameters and performances
save(fullfile(resultPath,['ResultsWparms_TLsource' testRunName '.mat']), ...
    'parms', ...
    'ktdReachingIndexAll','sucessAll','trialIndex', ...
    'QAll','kernelBWtraceAll', ...
    'successRate', ...
    'TLunit','TLweight','TLepochLength')
%% End of the script
